...
-(void) setupDepthBuffer {
    glGenRenderbuffers(1, &_depthRenderBuffer); //color和depth都属于renderbuffer的一种类型
...
- (void)compileShaders {
    ...
    _textCoordSlot = glGetAttribLocation(programHandle, "TexCoordIn");
...
-(void)render:(CADisplayLink *)displayLink {
    ...
    glVertexAttribPointer(_textCoordSlot, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (GLvoid*) (sizeof(float) * 7));
...
-(GLuint) setupTexture:(NSString *)fileName {
    ...
    //kCGImageAlphaPremultipliedLast定义了每一份数据中各个component的意义是什么
    //而每个component的大小则由8表示
    //每一份数据的大小则是由一行的总数据除去一行的个数
    //这些都仅仅只是对Context进行设置，告诉它之后执行的过程中应该怎样分析并取得数据
    //ColorSpace指的是RGB，CMYK这些规定颜色应该如何划分取值的准则
    //提供的data是给context darwing后存放渲染结果的，也就是说，图片转换成bit的结果存放在这里
    //如果是通过context绘画图片到内存的话，是选择bigmapContext（还有其他形式包括绘画到window，pdf等）
